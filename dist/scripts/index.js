...
var DynamicPropertyDB = class _DynamicPropertyDB {
  constructor(id) {
    this.data = {};
    this.keys = {};
    this.id = id;
  ...
  static {
    this.chunkLength = 1024 ** 2 * 8;
  }
  init() {
    let ids = world.getDynamicPropertyIds().filter((id) => id.startsWith(this.id)).map((key) => key.split("~"));
    while (ids.length) {
      let id = ids.pop();
  ...
  split(string) {
    if (string.length <= _DynamicPropertyDB.chunkLength)
      return [string];
    let i = 0;
    let out = [];
    while (i <= string.length)
      out.push(string.slice(i, i += _DynamicPropertyDB.chunkLength));
    return out;
  }
  parse() {
    for (let key in this.keys) {
...
var PathFinder = class {
  ...
  init() {
    world2.beforeEvents.worldInitialize.subscribe((event) => {
